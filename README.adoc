...
(f/ui-button #js {:content       "Like"
                  :icon          i/heart-icon
                  :label         #js {:as "a" :basic true :pointing "right" :content "2,000"}
                  :labelPosition "left"})
...
```
...
Props are required, and since these are just js wrapper the props that you send in must be js objects (e.g. prefix
with `#js`). Note that the "nested" elements with dot notation become just hyphenated names (e.g.
`Button.Group` -> `ui-button-group`, and `List.List` -> `ui-list-list`):
